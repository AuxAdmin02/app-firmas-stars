...
def crear_carpeta_drive(service, nombre_carpeta, id_padre=None):
    ...
    if items:
        st.write(f"Carpeta encontrada: {items[0]['name']}") # DEBUG
        return items[0]['id']
    else:
        file_metadata = {
            'name': nombre_carpeta,
        ...
        st.write(f"Carpeta creada: {folder.get('id')}") # DEBUG
        ...
        if st.button("Generar PDF y Subir a Drive", type="primary"):
            if canvas_result.image_data is not None:
                try:
                    ...
                    st.write(f"PDF generado, tamaño: {len(pdf_bytes)} bytes") # DEBUG
                    ...
                    file_id, file_link = subir_a_drive(service, pdf_bytes, nombre_pdf, 'application/pdf', id_carpeta_ref)

                    st.success(f"PDF guardado en Drive: Fotos_Anden > {st.session_state.datos_registro['ref_oddo']}")
                    st.link_button("Abrir PDF en Drive", file_link)
                    st.balloons()
